...
export function cleanConfig(themeConfig) {
    ...
    Object.keys(themeConfig).forEach(themeSection => {
        if (!themeTemplate[themeSection]) {
            console.warn(
                `React-able theme error: "${
                    themeSection
                }" not a valid config section. These values will not be set. Please use Themer template: `,
                themeTemplate
            );
        } else {
            Object.keys(themeConfig[themeSection]).forEach(sectionKey => {
                ...
                if (!templateSectionKey) {
                    console.warn(
                        `React-able theme error: "${
                            sectionKey
                        }" not a valid config key. This value will not be set. Please use Themer template: `,
                        themeTemplate
                    );
                } else {
